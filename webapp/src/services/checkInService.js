...
import { limitToFirst, orderByChild, query as fbQuery, startAt, endAt } from 'firebase/database'
...
const queryCheckIns = async ({ order, limit, minDate, maxDate, keywords = [] }) => {
    minDate -= GRACE_PERIOD;
    try {
        const query = fbQuery(dbRef, orderByChild('time'), limitToFirst(limit), startAt(minDate.valueOf()), endAt(maxDate.valueOf()))
        ...
        snapshot.forEach((snapChild) => {
            ...
            if (
                keywords.length === 0 ||
                keywords.some((keyword) => Object.values(checkIn).some((value) => value.includes(keyword)))
            ) {
